...
const populateUserData = (user) => {
  return user.populate('uploadedMusic').populate({
    path: 'likes',
    populate: {
      path: 'music',
      populate: {
...
exports.login = catchAsync(async (req, res, next) => {
  const querry = User.findOne({ name: req.body.name }).select('+password'); // добавляем поле password, которое не возвращается по умолчанию

  const user = await populateUserData(querry);

  if (
    !user ||
    !(await user.correctPassword(req.body.password, user.password))
  ) {
    return next(new AppError('Incorrect name or password', 400));
  }
...
exports.checkAuth = catchAsync(async (req, res, next) => {
  ...
  const querry = User.findById(decoded.id);
  const currentUser = await populateUserData(querry);
  if (!currentUser) {
...
exports.signup = catchAsync(async (req, res) => {
  ...
  newUser = await newUser.populate('uploadedMusic').populate({
    path: 'likes',
    populate: {
      path: 'music',
      populate: {
